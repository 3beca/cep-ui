...
export const setupNock = (url: string) => {
    const server = nock(url).defaultReplyHeaders({
...
export const generateListWith = function generateListWith(
    many: number = 5,
    next = false,
    prev = false,
    key: string = '_' + many
...
export const generateEventTypeListWith = (
    many: number = 5,
    next = false,
    prev = false,
    key: string = '_' + many,
...
export const generateTargetListWith = (
    many: number = 5,
    next = false,
    prev = false,
    key: string = '_' + many,
...
const randomType = (index: number): { type: RuleTypes; group?: RuleGroup; windowSize?: WindowingSize } => {
    ...
    switch (rand) {
        case 0:
            return {
                type: 'sliding',
                group: { maxTemp: { _max: 'temperature' } },
...
export const generateWindowingRule = (
    name: string,
    type: Exclude<RuleTypes, 'realtime'>,
    eventTypeId: string,
...
export const generateRuleListWith = function generateListWith(
    many: number = 5,
    next = false,
    prev = false,
    key: string = '_' + many
...
export const generateEventLogListWith = function generateListWith(
    many: number = 5,
    next = false,
    prev = false,
    key: string = '_' + many
...
export const serverDelete = (
    ...
    id: string,
    status: number = 204,
    response: undefined | ServiceError = undefined
) => {
...
export const serverGetEventTypeList = (
    server: nock.Scope,
    page: number = 1,
    size: number = 10,
    filter = '',
...
export const serverDeleteEventType = (
    ...
    eventTypeId: string,
    status: number = 204,
    response: undefined | EventTypeError = undefined
) => {
    return serverDelete(server, EVENT_TYPES_URL, eventTypeId, status, response);
};
export const serverCreateEventType = (
    server: nock.Scope,
    body: Partial<EventType>,
    status: number = 200,
...
export const serverDeleteTarget = (
    ...
    targetId: string,
    status: number = 204,
    response: undefined | TargetError = undefined
) => {
...
export const serverGetListAuth = function serverGetList<T>(
    server: nock.Scope,
    path: string,
    key: string,
    page: number = 1,
...
export const serverCreateAuth = <T>(
    server: nock.Scope,
    path: string,
    key: string,
    body: string,
