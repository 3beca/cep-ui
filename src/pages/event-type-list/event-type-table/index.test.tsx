...
test('EventTypeList snapshot', async () => {
    ...
    expect(
        await screen.findAllByLabelText(/element row eventtype/)
    ).toHaveLength(10);
});
...
runPaginatedTableTest(
    ...
    (
        ...
        pageSize: number = 10,
        status: number = 200,
        response: EventTypeList | EventTypeError
    ) =>
        serverGetEventTypeList(
            setupNock(BASE_URL),
            page,
...
runDeletableTableTest(
    ...
    (id: string, status: number = 200, response?: EventTypeError) =>
        serverDeleteEventType(setupNock(BASE_URL), id, status, response)
);
