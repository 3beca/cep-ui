...
import {
    ENTITY
} from '../../../services/api-provider/use-api';
...
export const TargetTable: React.FC<TargetTableProps> = ({
    ...
    // Selector
    const {selectOne, selectAll, selecteds} = useSelectableList(TargetList?.results, onSelected);
        ...
        <>
            <TitleTable title='List of Targets'>
                <IconDialog
                    ...
                    <DeleteDialog title='Delete Targets' entity={ENTITY.TARGETS} elementsSelecteds={[...selecteds]} onDeleted={forceReload}/>
                </IconDialog>
            </TitleTable>
            <PaginatedTable
                page={page}
                    ...
                    <TableBody>
                        {
                            targets.map((target: Target) => (
                                <TableRow key={target.id} aria-label='element row target'>
                                    <TableCell padding='checkbox'>
                                        <Checkbox
                                            inputProps={{role: 'element-selector'}}
                                            checked={selecteds.has(target)}
                                            onChange={(ev, checked) => selectOne(checked, target)}/>
                                    </TableCell>
                                    <TableCell align='left' aria-label='element name'>{target.name}</TableCell>
                                    <TableCell align='right'>{new Date(target.createdAt).toLocaleString()}</TableCell>
                                    <TableCell align='right'>
                                        <IconDialog
                                                show={true}
                                                aria-label='delete one dialog'
                                                icon={<DeleteIcon aria-label='delete one icon'/>}>
                                                <DeleteDialog title='Delete Targets' entity={ENTITY.TARGETS} elementsSelecteds={[target]} onDeleted={forceReload}/>
                                            </IconDialog>
                                    </TableCell>
                                </TableRow>
                            ))
