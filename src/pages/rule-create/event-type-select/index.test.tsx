...
test('EventTypeSelector should render a filtered by element options when type element and close keep selection', async () => {
    ...
    const prefix = 'test';
    const filteredResult = generateEventTypeListWith(
        5,
        false,
        false,
        '',
...
test('EventTypeSelector should select the third element from the options and change to details view, copy url and clear selection', async () => {
    ...
    const { rerender } = render(
        <EventTypeSelector selected={null} onSelected={setSelected} />
    );
    ...
    expect(screen.queryAllByRole('option')).toHaveLength(0);
    await screen.findByLabelText(/loading eventtypes/i);
    expect(await screen.findByLabelText(/search a eventtype/i)).toHaveAttribute(
        'value',
        'test2 EventType2'
    );
    ...
    rerender(
        <EventTypeSelector selected={fakeEventType} onSelected={setSelected} />
    );
    act(() => void jest.runOnlyPendingTimers());
    expect(
        await screen.findByLabelText(/eventtype selected name/i)
    ).toHaveTextContent(fakeEventType.name);
    expect(
        await screen.findByLabelText(/eventtype selected url/i)
    ).toHaveTextContent(fakeEventType.url);
...
test('EventTypeSelector should create new element when no options found', async () => {
    ...
    rerender(
        <EventTypeSelector
            selected={{ ...emptyEventType, name: prefix }}
