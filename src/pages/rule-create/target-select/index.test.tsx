...
test('TargetSelector should render a filtered by element options when type element and close keep selection', async () => {
    ...
    const prefix = 'test';
    const filteredResult = generateTargetListWith(
        5,
        false,
        false,
        '',
...
test('Target should select the third element from the options and change to details view, clear selection and back to search view', async () => {
    ...
    const { rerender } = render(
        <TargetSelector selected={null} onSelected={onSelected} />
    );
    ...
    await screen.findByLabelText(/loading targets/i);
    expect(await screen.findByLabelText(/search a target/i)).toHaveAttribute(
        'value',
        'test2 Target2'
    );
    ...
    ).toHaveTextContent(fakeTarget.name);
    expect(
        await screen.findByLabelText(/target selected url/i)
    ).toHaveTextContent(fakeTarget.url);
...
test('TargetSelector should create new element when no options found', async () => {
    ...
    rerender(
        <TargetSelector
            selected={{ ...emptyTarget, name: prefix }}
    ...
    ).toHaveTextContent(/creating target/i);
