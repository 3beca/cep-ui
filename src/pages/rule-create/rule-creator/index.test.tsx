...
test('RuleCreator should render disabled', async () => {
    ...
    expect(await screen.findByLabelText(/rule creator name/i)).toHaveValue(
        ruleHeader.name
    );
...
test('RuleCreator should create a new rule', async () => {
    ...
    const { rerender } = render(
        <RuleCreator
            ruleHeader={{ name: '', skipOnConsecutivesMatches: false }}
            updateRuleHeader={updateRuleHeader}
        />
    );
    ...
    [...ruleName].map((ch, index) =>
        expect(updateRuleHeader).toHaveBeenNthCalledWith(index + 1, {
            name: ch
        })
    );
    userEvent.click(
    ...
    expect(updateRuleHeader).toHaveBeenNthCalledWith(10, {
        skipOnConsecutivesMatches: true
    });

    rerender(
        <RuleCreator
            ruleHeader={{ name: ruleName, skipOnConsecutivesMatches: true }}
            updateRuleHeader={updateRuleHeader}
        />
    );
    userEvent.click(
    ...
    expect(updateRuleHeader).toHaveBeenNthCalledWith(11, {
        skipOnConsecutivesMatches: false
    });
});
