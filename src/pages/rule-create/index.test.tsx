...
jest.mock('react-router-dom', () => {
    ...
    const Link = React.forwardRef((props: any, ref: any) => {
        return (
            <div ref={ref} {...props} onClick={() => linkAction(props.to)} />
        );
    });
...
test('RuleCreatePage for realtime rules should render 3 sections, Manage EventTypes, Create rule and Manage Target and snapshot', async () => {
    ...
    serverGetTargetList(
        ...
        '',
        200,
        generateTargetListWith(10, false, false)
    );
...
test('RuleCreatePage should create a Passthrow rule', async () => {
    ...
    await userEvent.type(
        await screen.findByLabelText(/rule creator name/i),
        ruleName
    );
    userEvent.click(
        await screen.findByLabelText(/rule creator skip consecutives/i)
    );
    expect(
        await screen.findByLabelText(/rule create button/i)
    ).not.toBeDisabled();
    ...
    expect(
        await screen.findByLabelText(/rule create success message/i)
    ).toHaveTextContent(`Rule ${ruleCreated.name} created successfully`);
    expect(
        screen.queryByLabelText(/rule create error message/i)
    ).not.toBeInTheDocument();
    userEvent.click(
        await screen.findByLabelText(/rule create success button details/)
    );
    expect(fakeLink.linkAction).toHaveBeenCalledTimes(1);
    expect(fakeLink.linkAction).toHaveBeenNthCalledWith(
        1,
        '/rules/details/123456789098'
    );
});
...
test('RuleCreatePage should show an error when create rule fails', async () => {
    ...
    expect(
        await screen.findByLabelText(/rule create error message/i)
    ).toHaveTextContent(ruleError.message);
    expect(
        screen.queryByLabelText(/rule create success message/i)
    ).not.toBeInTheDocument();
});
...
test('RuleCreatePage should create a new Rule after create one', async () => {
    ...
    expect(await screen.findByLabelText(/rule create button/)).toBeDisabled();
    userEvent.click(
        await screen.findByLabelText(/rule create success button more/)
    );
    expect(fakeLink.linkAction).toHaveBeenCalledTimes(0);
    expect(
        screen.queryByLabelText(/rule create success$/i)
    ).not.toBeInTheDocument();
    expect(
        await screen.findByLabelText(/rule create button/)
    ).not.toBeDisabled();
...
test('RuleCreatePage for realtime rules should clear payload when change eventype', async () => {
    ...
    // Download payload
    const checkPayload = await screen.findByLabelText(
        /payload download button/i
    );
    ...
    await waitFor(() =>
        expect(
            screen.queryByLabelText(/payload creator loading/i)
        ).not.toBeInTheDocument()
    );
    ...
    expect(
        screen.queryByLabelText(/payload creator schema/i)
    ).not.toBeInTheDocument();
...
test('RuleCreatePage for realtime rules should overwrite payload fields with the same name', async () => {
    ...
    const addFieldButton = await screen.findByLabelText(
        /payload addfield button open dialog/i
    );
    ...
    await userEvent.type(
        await screen.findByLabelText(/payload addfield dialog name/),
        stringFieldName
    );
    userEvent.click(
        await screen.findByLabelText(/payload addfield dialog string/i)
    );
    userEvent.click(
        await screen.findByLabelText(/payload addfield dialog add/i)
    );
    ...
    userEvent.click(
        await screen.findByLabelText(/payload addfield dialog number/i)
    );
    userEvent.click(
        await screen.findByLabelText(/payload addfield dialog add/i)
    );
...
const enableRuleFilterComponent = async (type?: RuleTypes) => {
    ...
    expect(
        screen.queryByLabelText(/filter action buttons/i)
    ).not.toBeInTheDocument();
    ...
    userEvent.click(
        await screen.findByLabelText(/payload addfield dialog location/i)
    );
    userEvent.click(
        await screen.findByLabelText(/payload addfield dialog add/i)
    );
    ...
    userEvent.click(
        await screen.findByLabelText(/payload addfield dialog close/i)
    );
    expect(
        screen.queryByLabelText(/payload addfield dialog$/i)
    ).not.toBeInTheDocument();
...
test('RuleCreatePage for realtime rules should update filter when change payload', async () => {
    ...
    let availableFields = await screen.findAllByLabelText(
        /config filter options/i
    );
    ...
    await screen.findByLabelText(/config filter operator selector/);
    await screen.findByLabelText(/config filter value/);
    const secondValueField = await screen.findByLabelText(
        /config filter value/
    );
    ...
    const deletePayloadButtons = await screen.findAllByLabelText(
        /payload field button remove/
    );
    userEvent.click(deletePayloadButtons[0]);

    // Check it has been removed
    expect(
        screen.queryByLabelText(/payload field myStringfield/i)
    ).not.toBeInTheDocument();
    await screen.findByLabelText(/payload field myNumericfield/i);
    await screen.findByLabelText(/payload field myLocationfield/i);
    ...
    filters.map(filter =>
        expect(filter).not.toHaveTextContent('myStringfield')
    );
});
...
test('RuleCreatePage for realtime rules should config filter with 3 expressions and create the rule', async () => {
    ...
    await userEvent.type(
        await screen.findByLabelText(/rule creator name/),
        ruleName
    );
...
test('RuleCreatePage for realtime rules should config filter with two AND containers inside a OR container and create rule', async () => {
    ...
    const firstFilter = within(
        await screen.findByLabelText(/container expressions$/i)
    );
    const nestedAndButton = await firstFilter.findByLabelText(
        /filter add button and/i
    );
    // Add first nested AND
    userEvent.click(nestedAndButton);
    const firstNestedAnd = within(
        await firstFilter.findByLabelText(/container expressions$/i)
    );
    // Add sencond nested AND
    userEvent.click(nestedAndButton);
    const secondNestedAnd = within(
        firstFilter.queryAllByLabelText(/container expressions$/i)[1]
    );
    // Create an expression inside first and nested
    userEvent.click(
        await firstNestedAnd.findByLabelText(/filter add button expression/i)
    );
    // Open Filter dialog
    await screen.findByLabelText(/config filter dialog/i);
    ...
    let availableOperators = await screen.findAllByLabelText(
        /config filter operators/i
    );
    ...
    userEvent.click(await screen.findByLabelText(/config filter button save/));
    expect(
        await firstNestedAnd.findByLabelText(/filter expression field/)
    ).toHaveTextContent('myStringfield');
    expect(
        await firstNestedAnd.findByLabelText(/filter expression value/)
    ).toHaveTextContent('temperatureSensor');
    // Create an expression inside second and nested
    userEvent.click(
        await secondNestedAnd.findByLabelText(/filter add button expression/i)
    );
    // Open Filter dialog
    await screen.findByLabelText(/config filter dialog/i);
    ...
    availableOperators = await screen.findAllByLabelText(
        /config filter operators/i
    );
    expect(availableOperators).toHaveLength(5);
    ...
    userEvent.click(await screen.findByLabelText(/config filter button save/));
    expect(
        await secondNestedAnd.findByLabelText(/filter expression field/)
    ).toHaveTextContent('myNumericfield');
    expect(
        await secondNestedAnd.findByLabelText(/filter expression value/)
...
test('RuleCreatePage for sliding rules should config the group, the windowing and the filter and create the rule', async () => {
    ...
    expect(
        screen.queryByLabelText(
            /rule group creator addfield dialog selector targets from payload/i
        )
    ).toBeInTheDocument();
    await userEvent.type(groupDialog.inputName, avgFieldName);
    expect(
        await screen.findByLabelText(/rule group creator addfield dialog add/i)
    ).toBeDisabled();
    // Set operator
    userEvent.click(groupDialog.selectOperator);
    expect(
        await screen.findAllByLabelText(
            /rule group creator addfield dialog select operator/i
        )
    ).toHaveLength(6);
    expect(
        (
    ...
    ).toHaveTextContent(/Average/i);
    await screen.findByLabelText(
        /rule group creator addfield dialog select operator _avg/i
    );
    userEvent.click(
        (
            await screen.findAllByLabelText(
    ...
    expect(
        await screen.findByLabelText(
            /rule group creator addfield dialog operators/i
        )
    ).toHaveTextContent(/Average/i);
    expect(
        await screen.findByLabelText(/rule group creator addfield dialog add/i)
    ...
    expect(
        screen.queryByLabelText(
            /rule group creator addfield dialog input value/i
        )
    ).not.toBeInTheDocument();
    ...
    expect(
        await screen.findAllByLabelText(
            /rule group creator addfield dialog select target/i
        )
    ).toHaveLength(1); // Exclude not numeric fields
    userEvent.click(
        (
            await screen.findAllByLabelText(
                /rule group creator addfield dialog select target/i
    ...
    expect(
        await screen.findByLabelText(
            /rule group creator addfield dialog container/i
        )
    ).toBeInTheDocument();
    expect(await screen.findByLabelText(/rule create button/)).toBeDisabled();
    ...
    ).toHaveTextContent(/Max. Value/i);
    await screen.findByLabelText(
        /rule group creator addfield dialog select operator _max/i
    );
    userEvent.click(
        (
            await screen.findAllByLabelText(
    ...
    userEvent.click(
        await screen.findByLabelText(
            /rule group creator addfield dialog close/i
        )
    );
    expect(
        screen.queryByLabelText(/rule group creator addfield dialog container/i)
    ).not.toBeInTheDocument();
    // Check group payload schema
    const groupFields = await screen.findAllByLabelText(
        /rule group creator payload schema field/i
    );
    ...
    expect(
        await screen.findAllByLabelText(/config filter options/i)
    ).toHaveLength(2);
    // Select avgNumeric
    userEvent.click(
        (await screen.findAllByLabelText(/config filter options/i))[0]
    );
    // Select GT operator
    userEvent.click(
        await screen.findByLabelText(/config filter operator selector/)
    ...
    expect(
        await screen.findAllByLabelText(/config filter operators/i)
    ).toHaveLength(5);
    userEvent.click(
        (await screen.findAllByLabelText(/config filter operators/i))[1]
    );
    ...
    // Select maxNumeric
    userEvent.click(
        (await screen.findAllByLabelText(/config filter options/i))[1]
    );
    // Select LT operator
    userEvent.click(
        await screen.findByLabelText(/config filter operator selector/)
    ...
    userEvent.click(
        (await screen.findAllByLabelText(/config filter operators/i))[3]
    );
    // Set value for expression avgNumeric
    const maxValue = 40;
    await userEvent.type(
    ...
    // Check filters
    expect(
        (await screen.findAllByLabelText(/filter expression field/))[0]
    ).toHaveTextContent(avgFieldName);
    expect(
        (await screen.findAllByLabelText(/filter expression operator/))[0]
    ).toHaveTextContent('>');
    expect(
        (await screen.findAllByLabelText(/filter expression value/))[0]
    ).toHaveTextContent(avgValue + '');
    expect(
        (await screen.findAllByLabelText(/filter expression field/))[1]
    ).toHaveTextContent(maxFieldName);
    expect(
        (await screen.findAllByLabelText(/filter expression operator/))[1]
    ).toHaveTextContent('<');
    expect(
        (await screen.findAllByLabelText(/filter expression value/))[1]
    ).toHaveTextContent(maxValue + '');
    expect(await screen.findByLabelText(/rule create button/)).toBeDisabled();

    // Set windowSize to 10 minutes
    await screen.findByLabelText(/rule windowsize main container/);
    userEvent.click(
        await screen.findByLabelText(/rule windowsize unit minute/)
    );
    await screen.findByLabelText(/rule windowsize unit minute selected/);
    await userEvent.type(
        await screen.findByLabelText(/rule windowsize input value/),
        '10'
    );
