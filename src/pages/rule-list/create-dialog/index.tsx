...
const subtitles: RuleTypesText = {
    ...
    tumbling: ['This rule executes in fixed time intervals aggregating events reveived inside each interval', 'As sliding rules, tumbling rules perform an aggregation of a given time window. However, in spite of realtime and slinding rules, tumbling rules got executed on a given time interval. This time interval matches the time window'],
    hopping: ['This rule executes each custom time interval aggregatting events occurred in a fixed time interval', 'compute windowing with...']
};
    ...
    return (
        <Dialog
            ...
            <DialogActions aria-label='actions'>
                ...
                <Button onClick={fireSelected} disabled={!type} className={styles.selectButton} aria-label='select button'>Select</Button>
            </DialogActions>
        </Dialog>
    );
};
