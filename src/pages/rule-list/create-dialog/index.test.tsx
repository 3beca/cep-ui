...
test('CreateRuleDialog should render dialog when click in icon dialog and close when press close button', async () => {
    ...
    await waitFor(() =>
        expect(
            screen.queryByLabelText(/create rule dialog/i)
        ).not.toBeInTheDocument()
    );
});

const runSelectCardTest = (
    ariaLabel: RegExp,
    typeSelected: RuleTypes | 'realtime'
) => {
    test(
        'CreateRuleDialog should render dialog when click in icon dialog and close when select ' +
            ariaLabel,
        async () => {
            const onClose = jest.fn();
