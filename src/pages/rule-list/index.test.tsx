...
export const selectCreateRule = async (
    ariaLabel: 'real time' | 'hopping' | 'sliding' | 'tumbling'
) => {
    ...
    const realTimeCard = await screen.findByLabelText(
        `create rule ${ariaLabel} card`
    );
    const selectButton = await screen.findByText(/^select$/i);
    userEvent.click(realTimeCard);
    userEvent.click(selectButton);
    await waitFor(() =>
        expect(document.getElementById('create-rule-dialog')).toBe(null)
    );
};
...
test('RuleListPage should render loading component', async () => {
    serverGetRuleList(
        ...
        filter,
        200,
        generateRuleListWith(initialPageSize, false, false)
    );
    ...
    expect(
        await screen.findAllByLabelText(/^element card rule$/i)
    ).toHaveLength(initialPageSize);
});
...
test('RuleListPage should render Empty List when no elements and snap', async () => {
    serverGetRuleList(
        ...
        filter,
        200,
        generateRuleListWith(0, false, false)
    );
    renderInsideApp(<RuleListPage />);

    expect(await screen.findByTestId(/empty-view-row/i)).toBeInTheDocument();
    expect(await screen.findByTestId(/empty-view-row/i)).toHaveTextContent(
        /there are no rules created yet/i
    );
    await screen.findByLabelText(/add rule/i);
});
...
test('RuleListPage should render End List message when no more elements to load and snap', async () => {
    ...
    expect(await screen.findByTestId(/empty-view-row/i)).toHaveTextContent(
        /you reached the end of the list/i
    );
    await screen.findByLabelText(/add rule/i);
});
...
test('RuleListPage should render 20 cards and a create button and snapshot', async () => {
    ...
    expect(await screen.findAllByLabelText(/filters card rule/i)).toHaveLength(
        initialPageSize
    );
    expect(await screen.findAllByLabelText(/status card rule/i)).toHaveLength(
        initialPageSize
    );
    await screen.findByLabelText(/add rule/i);
});
...
test('RuleListPage should render a create rule dialog when click on add button and close when press close button', async () => {
    serverGetRuleList(
        ...
        filter,
        200,
        generateRuleListWith(1, false, false)
    );
    renderInsideApp(<RuleListPage />);
...
test('RuleListPage should render a searchbar  and find rules that contains rule', async () => {
    ...
    serverGetRuleList(
        ...
        searchText,
        200,
        generateRuleListWith(5, false, false)
    );
    ...
    await waitFor(() =>
        expect(
            screen.queryByTestId(/loading-view-row/i)
...
test('RuleListPage should render Empty List whith filter when no elements and snap', async () => {
    ...
    expect(await screen.findByTestId(/empty-view-row/i)).toBeInTheDocument();
    expect(await screen.findByTestId(/empty-view-row/i)).toHaveTextContent(
        /There are no elements for "rule-name"/i
    );
    jest.useRealTimers();
});
...
test('RuleListPage should render load button when there are more elements and load next set of elelments when click', async () => {
    serverGetRuleList(
        ...
        filter,
        200,
        generateRuleListWith(initialPageSize, true, false)
    );
    renderInsideApp(<RuleListPage />);
    ...
    serverGetRuleList(
        ...
        filter,
        200,
        generateRuleListWith(5, false, true)
    );
...
test('RuleListPage should render 10 RuleCards with header eventType, target, filters and status', async () => {
    ...
    expect(await screen.findAllByLabelText(/element card rule/i)).toHaveLength(
        10
    );
    expect(
        await screen.findAllByLabelText(/eventType name card rule/i)
    ).toHaveLength(10);
    expect(
        await screen.findAllByLabelText(/target name card rule/i)
    ).toHaveLength(10);
    expect(await screen.findAllByLabelText(/filters card rule/i)).toHaveLength(
        10
    );
...
test('RuleListPage should delete the first rule and update the list', async () => {
    ...
    const dialogs = await screen.findAllByLabelText(
        /setting dialog card rule visible/i
    );
    expect(dialogs).toHaveLength(1);
    const actionsDialog = within(dialogs[0]);
    // Open delete action
    userEvent.click(
        await actionsDialog.findByLabelText(/setting dialog delete card rule/i)
    );
    const deleteDialogs = await screen.findAllByLabelText(
        /delete dialog card rule/i
    );
...
test('RuleListPage should delete the first rule when load two pages and update the list', async () => {
    ...
    serverGetRuleList(
        ...
        filter,
        200,
        generateRuleListWith(5, false, true, 'page2')
    );
    userEvent.click(loadMore);
    expect(await screen.findByTestId(/loading-view-row/i)).toBeInTheDocument();
    await waitFor(() =>
