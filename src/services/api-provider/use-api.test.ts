...
import { useGetList, useDelete, useCreate, ENTITY } from './use-api';
import { BASE_URL } from '../config';
import { ServiceError, EventType, Target, TargetError } from '../api';
import {
    setupNock,
    generateEventType,
...
describe('useGetList', () => {
    ...
    it('should force a reload when fire request', async () => {
        let page = 1;
        let size = 3;
        const expectedResult = generateEventTypeListWith(3, true, false);
        serverGetEventTypeList(server, page, size, '', 200, expectedResult);
    ...
    it('should force a reload when change filter', async () => {
        let page = 1;
        let size = 3;
        let filter = '';
        const expectedResult = generateEventTypeListWith(3, true, false);
    ...
    it('should NOT fire request when load is false', async () => {
        let page = 1;
        let size = 3;
        const expectedResult = generateEventTypeListWith(3, true, false);
        serverGetEventTypeList(server, page, size, '', 200, expectedResult);
    ...
    it('should receive a a list of one element when using fiter eventTypeId', async () => {
        const page = 1;
        const size = 1;
...
describe('useDelete', () => {
    it('shuold delete one EventType and return OK', async () => {
        ...
        expect(result.current.response).toEqual({
            status: 200,
            data: [{ id: eventTypeId, state: 'DELETED' }]
        });
    });

    it('should try to delete one invalid EventType and return Error', async () => {
    ...
    it('shuold delete a list of EventTypes and return its response', async () => {
        const eventTypeIds = ['123456789098', '123456456789', '153478904567'];
        const errorResponse: ServiceError = {
            statusCode: 500,
            error: 'eventId can not be deleted',
            message: 'Error deleteing eventId'
        };
...
describe('useCreate', () => {
    ...
    it('should create a new Target', async () => {
        ...
        const targetBody: Partial<Target> = {
            name: target.name,
            url: target.url
        };
    ...
    it('should fails to create new Target', async () => {
        const targetBody: Partial<Target> = {
            name: 'existing name',
            url: 'http://anywhere'
        };
    ...
    it('should create a new Target on Render', async () => {
        let target = generateTarget(1, 'usetCreateTest', 'ttest');
        let targetBody: Partial<Target> = {
            name: target.name,
            url: target.url
        };
        serverCreateTarget(server, targetBody, 201, target);
