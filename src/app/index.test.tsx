...
test('App render RulesPage with route / and snapshot', async () => {
    ...
    await waitFor(() =>
        expect(
            screen.queryByLabelText(/^drawer menu$/i)
...
test('App render / and navigate to Event Types Page with route /event-types and snapshot', async () => {
    ...
    expect(
        await screen.findAllByLabelText(/element row eventtype/i)
    ).toHaveLength(10);
...
test('App render / and navigate to TargetListPage with route /targets and snapshot', async () => {
    ...
    expect(await screen.findAllByLabelText(/element row target/i)).toHaveLength(
        10
    );
    // expect drawer will be closed after navigate to page
    await waitFor(() =>
...
test('App render / and navigate to Events Log with route /event-logs and snapshot', async () => {
    ...
    expect(
        await screen.findAllByLabelText(/^element row event logs$/i)
    ).toHaveLength(10);
    // expect drawer will be closed after navigate to page
    await waitFor(() =>
        expect(
...
test('App render / and navigate to rule details', async () => {
    ...
    const firstRuleCardSettings = (
        await screen.findAllByLabelText(/settings card rule/i)
    )[0];
    userEvent.click(firstRuleCardSettings);

    expect(
        await screen.findAllByLabelText(/setting dialog card rule/i)
    ).toHaveLength(10);
    const detailsCardButtons = await screen.findAllByLabelText(
        /setting dialog details card rule/i
    );
