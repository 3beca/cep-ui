...
test(
    'RuleGroupCreator should show a message when EventPayload exist but group needs to be created',
    async () => {
...
test(
    'RuleGroupCreator should open add field dialog with add button disabled and close dialog',
    async () => {
        const setGroup = jest.fn();
...
test(
    'RuleGroupCreator should create only numeric sum group when no payload',
    async () => {
...
test(
    'RuleGroupCreator should filter non numeric payload fields',
    async () => {
        const fieldName = 'countEvents';
...
test(
    'RuleGroupCreator should create a group with max temperature',
    async () => {
...
test(
    'RuleGroupCreator should add average humidity a group with max temperature',
    async () => {
        ...
        await userEvent.type(inputName, fieldName);
        expect(await screen.findByLabelText(/rule group creator addfield dialog add/i)).toBeDisabled();
        expect(await screen.findByLabelText(/rule group creator addfield dialog selector targets from payload/i)).toHaveAttribute('aria-disabled', 'true');
        userEvent.click(selectOperator);
        let operators = await screen.findAllByLabelText(/rule group creator addfield dialog select operator/i);
        expect(operators).toHaveLength(6);
        ...
        expect(await screen.findByLabelText(/rule group creator addfield dialog selector targets from payload/i)).not.toHaveAttribute('aria-disabled', 'true');
...
test(
    'RuleGroupCreator should not duplicate names',
    async () => {
        const prevGroup: RuleGroupPayload = [
            {field: 'temperature', operator: '_max', name: 'maxTemperature'}
        ];
...
test(
    'RuleGroupCreator should change summation from field to number',
    async () => {
        const prevGroup: RuleGroupPayload = [
...
test(
    'RuleGroupCreator should change summation between number and field',
    async () => {
        const prevGroup: RuleGroupPayload = [
        ...
        expect(await screen.findByLabelText(/rule group creator addfield dialog selector targets from payload/i)).toHaveTextContent('temperature');
...
test(
    'RuleGroupCreator add dialog should create a group that sum 1 on each event',
    async () => {
        const fieldName = 'countEvents';
        const target = 1;
        const operator = '_sum';
...
test(
    'RuleGroupCreator add dialog should create a group for max and min temperature',
    async () => {
        const setGroup = jest.fn();
        const {rerender} = render(<RuleGroupCreator ruleTpe='sliding' payload={validEventPayload} setGroup={setGroup}/>);
