...
test('TargetValue should only accept positive integers values', async () => {
    ...
    const inputValue = await screen.findByLabelText(
        /rule group creator addfield dialog input value/i
    );

    fireEvent.change(inputValue, { target: { value: 'notnumber' } });
    await screen.findByLabelText(
        /rule group creator addfield dialog input error value/i
    );
...
test('RuleGroupCreator should show a message when EventPayload exist but group needs to be created', async () => {
    const group: RuleGroupPayload = [
        { name: 'fieldName', operator: '_sum', field: 1 }
    ];
    ...
    await screen.findByLabelText(
        /rule group creator addfield button open dialog/i
    );
    await screen.findByLabelText(
        /rule group creator info group payload message/
    );
    ...
    expect(
        screen.queryByLabelText(/rule group creator info group payload message/)
    ).not.toBeInTheDocument();
});
...
export const openAddFieldGroupDialog = async () => {
    ...
    const inputName = await screen.findByLabelText(
        /rule group creator addfield dialog name/i
    );
    const selectOperator = await screen.findByLabelText(
        /rule group creator addfield dialog operators/i
    );
    await screen.findByLabelText(/rule group creator addfield dialog targets/i);
...
test('RuleGroupCreator should create only numeric sum group when no payload', async () => {
    ...
    ).toHaveTextContent(/summation/i);
    expect(
        await screen.findByLabelText(/rule group creator addfield dialog add/i)
    ).toBeDisabled();
    const numericInput = await screen.findByLabelText(
        /rule group creator addfield dialog input value/i
    );
    ...
    const expectedGroup: RuleGroupPayload = [
        { field: target, name: fieldName, operator }
    ];
...
test('RuleGroupCreator should create a group with max temperature', async () => {
    ...
    const targets = await screen.findAllByLabelText(
...
test('RuleGroupCreator should add average humidity a group with max temperature', async () => {
    const prevGroup: RuleGroupPayload = [
...
test('RuleGroupCreator should change summation from field to number', async () => {
    const prevGroup: RuleGroupPayload = [
        { field: 'temperature', operator: '_sum', name: 'maxTemperature' }
    ];
    const fieldName = 'maxTemperature';
    const operator = '_sum';
    const target = 1;
...
test('RuleGroupCreator should change summation between number and field', async () => {
    const prevGroup: RuleGroupPayload = [
        { field: 1, operator: '_sum', name: 'maxTemperature' }
    ];
    const fieldName = 'maxTemperature';
    const operator = '_sum';
    const target = 15;
...
test('RuleGroupCreator add dialog should create a group for max and min temperature', async () => {
    ...
    userEvent.click(selectOperator);
    let operators = await screen.findAllByLabelText(
        /rule group creator addfield dialog select operator/
    );
    expect(operators).toHaveLength(6);
    userEvent.click(operators[0]);
    expect(
    ...
    let fields = await screen.findAllByLabelText(
        /rule group creator addfield dialog select target/i
    );
    ...
    const expectedGroupMax: RuleGroupPayload = [
        { field: 'temperature', operator: '_max', name: 'maxTemperature' }
    ];
    ...
    rerender(
        <RuleGroupCreator
            ...
            payload={validEventPayload}
            setGroup={setGroup}
            group={expectedGroupMax}
        />
    );

    expect(
        screen.queryByLabelText(/rule group creator info grouppayload message/)
    ).not.toBeInTheDocument();
    ...
    userEvent.click(selectOperator);
    operators = await screen.findAllByLabelText(
        /rule group creator addfield dialog select operator/
    );
    expect(operators).toHaveLength(6);
    ...
    fields = await screen.findAllByLabelText(
        /rule group creator addfield dialog select target/i
    );
    expect(fields).toHaveLength(3);
    userEvent.click(fields[0]);
    ...
    rerender(
        <RuleGroupCreator
            ...
            payload={validEventPayload}
            setGroup={setGroup}
            group={expectedGroupCompleted}
        />
    );
    await screen.findByLabelText(
        /rule group creator payload schema container/i
    );
});
...
test('RuleGroupCreator do not show schema while not group', async () => {
    ...
    expect(
        screen.queryByLabelText(/rule group creator payload schema container/i)
    ).not.toBeInTheDocument();
});
...
test('RuleGroupCreator should remove maxTemperature from group', async () => {
    ...
    const deleteButtons = await screen.findAllByLabelText(
        /rule group creator schema field button remove/i
    );
    ...
    expect(setGroup).toHaveBeenNthCalledWith(
        1,
        expectedGroupWithoutTemperature
    );
});
...
test('RuleGroupCreator should keep in sync payload and group when deleting all fields', async () => {
    ...
    const expectedGroupOnlySummation: RuleGroupPayload = [
        { field: 1, operator: '_sum', name: 'countEvents' }
    ];
