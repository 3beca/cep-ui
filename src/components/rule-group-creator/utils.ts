...
export const syncEventPayloadAndGroupPayload = (
    ...
    for (const groupField of group) {
        if (payloadNames.includes(groupField.field + '') || (groupField.operator === '_sum' && Number.isInteger(groupField.field))) {
...
export const parseRuleGroupPayloadToRuleGroup = (group: RuleGroupPayload | undefined): RuleGroup | undefined => {
    ...
    const ruleGroup: RuleGroup = group.reduce<RuleGroup>((newGroup, payloadField) => {
        if (payloadField.name)
            return {
                ...newGroup,
                [payloadField.name]: {
                    [payloadField.operator]: typeof payloadField.field === 'number' ? payloadField.field : '_' + payloadField.field
                }
            } as RuleGroup;
        return newGroup;
    }, Object.create(null));
