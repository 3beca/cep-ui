...
import { buildPayloadFromEventLogPayload, EventPayloadFieldTypes, EventPayload, EventPayloadField } from './models';
...
export const PayloadFieldViewDeleteButton: React.FC<PayloadFieldViewDeleteButtonProps> = ({ removePayloadField, disabled = false }) => {
    ...
    return (
        <IconButton aria-label='payload field button remove' onClick={removePayloadField} disabled={disabled}>
            <DeleteIcon />
        </IconButton>
    );
};
...
export const PayloadFieldView: React.FC<PayloadFieldViewProps> = ({ payloadField, removePayloadField, disabled }) => {
    ...
    return (
        <div aria-label={`payload field`} className={styles.paloadFieldView}>
            ...
            <Typography className={styles.payloadFieldViewText} aria-label={`payload field ${payloadField.name}`}>
                {payloadField.name}
            </Typography>
...
export const PayloadSchema: React.FC<PayloadSchemaProps> = ({ eventTypeId, disabled, payload, setPayload }) => {
    ...
    if (!eventTypeId) {
        return (
            <div aria-label='payload creator info message no eventTypeId' className={styles.info}>
    ...
    if (!payload) {
        return (
            <div aria-label='payload creator info message no payload' className={styles.info}>
...
export const PayloadAddField: React.FC<PayloadAddFieldProps> = ({ disabled, updatePayload }) => {
    ...
    return (
        <>
                ...
                <div className={styles.addFieldForm}>
                    <div className={styles.addFieldName}>
                        <TextField
                            ...
                            label='Field name'
                            placeholder='Set a field name'
                        />
                    </div>
                    <div className={styles.addfieldSelector}>
                        <Typography className={styles.addfieldSelectorLabel} variant='caption'>
                            Select a field type
                        </Typography>
            ...
            <DialogActions>
                <Button aria-label='payload addfield dialog close' onClick={closeDialog}>
                    Close
                </Button>
                <Button onClick={addField} disabled={isDisabled} className={styles.selectButton} aria-label='payload addfield dialog add'>
                    Add
                </Button>
            </DialogActions>
        </>
    ...
    return (
        <div className={styles.container} aria-label={`payload creator${disabled ? ' disabled' : ''}`}>
            <div className={styles.payloadCreatorHeader}>
                <Typography variant='caption' className={styles.payloadCreatorHeaderTitle}>
                    Event payload
                </Typography>
                <IconDialog
                ...
                <PayloadDownloader eventTypeId={eventTypeId} disabled={disabled} setPayload={setPayload} />
            </div>
            <PayloadSchema eventTypeId={eventTypeId} disabled={disabled} payload={payload} setPayload={setPayload} />
        </div>
    );
};
