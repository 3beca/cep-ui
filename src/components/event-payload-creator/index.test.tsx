...
test('PayloadCreator should show a button to try to download the las payload from an event without payload', async () => {
    ...
    const closeDialog = await screen.findByLabelText(
        /payload creator help button close/i
    );
    userEvent.click(closeDialog);
    expect(
        screen.queryByLabelText(/payload creator help$/i)
...
test('PayloadCreator should delete fields from payload', async () => {
    ...
    const buttonRemovePayloadField = await screen.findAllByLabelText(
...
test('PayloadCreator should create a payload by hand', async () => {
    ...
    expect(
        screen.queryByLabelText(/paylaod addfield dialog/i)
    ).not.toBeInTheDocument();

    // Rerender with 3 fields
    const updatedPayload = [
        ...expectedLocationPayload,
...
test('PayloadCreator should add new fields to a payload', async () => {
    ...
    expect(setPayload).toHaveBeenNthCalledWith(1, [
        ...payload,
        ...expectedStringPayload
    ]);
});
...
test('PayloadFieldView can hidde delete button when no action', async () => {
    ...
    expect(
        screen.queryByLabelText(/payload field button remove/i)
    ).not.toBeInTheDocument();
});
